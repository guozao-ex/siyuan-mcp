...
export function createCache<T>(options?: Partial<CacheOptions>): Cache<T> {
  ...
  setInterval(() => {
    ...
    if (removed > 0) {
      // 必须写 stderr：stdio 传输模式下 stdout 是 MCP 协议的专用通道，
      // 而这里是定时器回调，往 stdout 写会随机插入到 JSON-RPC 消息之间。
      // 详见 enhanced-logger.ts 中关于 stderrLevels 的说明。
      console.error(`Cache cleanup: removed ${removed} expired entries`);
    }
  }, 60 * 1000);
